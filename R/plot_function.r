...
multi.plot <- function(data, ans1, type = "abundance"){
  ...
  p0 = draw.f_Div(data1,ans1[[1]])
  p1 = draw.f_Div(data1,ans1[[2]])
  p2 = draw.f_Div(data1,ans1[[3]])
  ...
  return(list(g1,p0_ana))
}



#' draw.f_Div_new(data, output, q) plot the outcome of Abundance(abundance data) or Incidence(incidence data).
#' @param data the Sx2 data used in Abundance/Incidence.
#' @param output matrix form:the outcome of mixture diversity for Abundance/Incidence data by q.
#' @param q q=0,1,2
#' @return one plot: for diversity of input q 
#' @import dplyr 
#' @export
draw.f_Div <- function(data1, output,type="abundance",q=0){
  print(type)
  
  if(ncol(output) == 8){
    output = output[,-ncol(output)]
  }
 # samey = max(output[,-c(1,2)])
  if(type == "abundance"){
    x1 = data1[, 1]
    x2 = data1[, 2]
    n1 =  sum(x1)
  ...
  #  xlim(c(0, max(output1$m)+5))+
    ...
    scale_fill_manual(values = c("black", "#3498DB","#C0392B"),
                      ...
                      labels = c(name1[1],name1[2],"Mixture"))
  if(n1==n2){
    pp=pp + geom_point(aes(output2$nn[1], output2$va[1]),colour="black",size=8)+
  ...
  pp<-pp+theme(plot.title = element_text(size = 20, face = "bold"))
  if(q==0){
    pp<-pp+ggtitle("q=0, species richness")
  }else if(q==1){
    pp<-pp+ggtitle("q=1")
  }else if(q==2){
...
#' plot_comb1(data1, output_d, output_p,q) plot species composition.
#' @param data the Sx2 data used in Abundance/Incidence.
#' @param output_d the outcome of mixture diversity for Abundance/Incidence data by q=0.
#' @param output_p the outcome of species composition by q=0.
#' @param q q=0: currently only for q=0
#' @return one plot: for diversity of input q 
#' @importFrom reshape2 melt
#' @import dplyr 
#' @export
plot_comb1 <- function(data1, output_d, output_p,type="abundance"){
  if(ncol(output_d) == 8){
    output_d = output_d[,-ncol(output_d)]
    output_p = output_p[,-((ncol(output_p) - 2): (ncol(output_p)))]
  }
  if(type == "abundance"){
    x1 = data1[, 1]
    x2 = data1[, 2]
  ...
  zz<-output_d[,c(1:3,6)]
  output_dzz = melt(data = output_d[,c(1:3,6)],id.vars = c("m1","lty"),variable.name = "col", value.name = "est")
  ...
  pp = ggplot(output)+
    ...
    scale_fill_manual(values=c("black", "#C0392B","black","#3498DB","purple"),
                      breaks = c(sites[1],"Mixture",p_names[3],p_names[1],p_names[2]))
  pp<-pp+ggtitle("q=0, species composition")+theme(plot.title = element_text(size = 20, face = "bold"))
